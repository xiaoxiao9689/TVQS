...
def measure_obs(N, prob, bits):
    n = len(bits)
    baseobs = get_baselocal(n)
    mat_prob = mat_psi(prob, bits)
    reduce_prob = np.sum(mat_prob, axis=1)
    result = np.dot(reduce_prob, baseobs)
    return result
...
def sqrt_iswap():
    XY2 = arb_twoXXgate(0, 1, 2) + arb_twoYYgate(0, 1, 2)
    sqrt_iwap = expm(-1j * (np.pi / 8) * XY2)
    return sqrt_iwap
